...
int curve_internal_fast_tests(void);
...
/**
 * Deserialize the public key from a buffer.
 * The format of the buffer corresponds to keys serialized with
 * ec_public_key_serialize().
 *
 * @param key Pointer to a buffer that will be allocated by this function
 *     and filled with the key. The caller is responsible for freeing this
 *     buffer with ec_public_key_destroy().
 * @param buffer The buffer with the serialized key
 * @return 0 on success, negative on failure
 */
int ec_public_key_deserialize(ec_public_key **key, signal_buffer *buffer);
