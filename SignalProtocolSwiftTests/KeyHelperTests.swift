...
class KeyHelperTests: XCTestCase {
    ...
    func testGenerateIdentityKeyPair() {
        // The commented bytes are the protobuf bytes
        let identityKeyPair: [UInt8] = [
        ...
        guard [UInt8](serialized) == identityKeyPair else {
    ...
    func testGeneratePreKeys() {
        ...
        guard let p1 = prekeys[1], let b1 = try? [UInt8](p1.data()), prekey1 == b1 else {
            XCTFail("Prekey 1 doesn't match")
            return
        }
        guard let p2 = prekeys[2], let b2 = try? [UInt8](p2.data()), prekey2 == b2 else {
            XCTFail("Prekey 2 doesn't match")
            return
        }
        guard let p3 = prekeys[3], let b3 = try? [UInt8](p3.data()), prekey3 == b3 else {
            XCTFail("Prekey 3 doesn't match")
            return
        }
        guard let p4 = prekeys[4], let b4 = try? [UInt8](p4.data()), prekey4 == b4 else {
    ...
    func testGenerateSignedPreKey() {
        ...
        guard let bytes = try? [UInt8](signed.data()), signedPreKey == bytes else {
            XCTFail("Signed key doesn't match")
            if let bytes = try? [UInt8](signed.data()) {
                print("Length \(signedPreKey.count), \(bytes.count)")
                for i in 0..<bytes.count {
                    if signedPreKey[i] != bytes[i] {
                        print("\(i): \(bytes[i]) not \(signedPreKey[i])")
                    }
                }
            }
            return
