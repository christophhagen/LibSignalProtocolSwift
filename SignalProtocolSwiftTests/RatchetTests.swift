...
class RatchetTests: XCTestCase {
    ...
    private func testChainKeyDerivation(
        version: HKDFVersion,
        messageKey: [UInt8],
        macKey: [UInt8],
        nextChainKey: [UInt8]) {
        ...
        let kdf = HKDF(messageVersion: version)
        let chainKey = RatchetChainKey(kdf: kdf, key: seed, index: 0)
    ...
    func testRootKeyDerivationV2() {
        let rootKeySeed: [UInt8] = [
        ...
        guard let alicePrivateKey = try? PrivateKey(point: alicePrivate),
            let bobPublicKey = try? PublicKey(point: bobPublic) else {
                XCTFail("Could not create keys")
                return
        }

        let rootKey = RatchetRootKey(kdf: HKDF(messageVersion: .version2), key: rootKeySeed)
        guard let (nextRootKey, nextChainKey) = try? rootKey.createChain(
            theirRatchetKey: bobPublicKey, ourRatchetKey: alicePrivateKey) else {
            XCTFail("Could not create next keys")
            return
        }

        guard rootKey.key == rootKeySeed,
            nextRootKey.key == nextRoot,
            nextChainKey.key == nextChain else {
                XCTFail("Keys not correct")
                return
        }
    ...
    func testIdentityKeySerialize() {
        ...
        guard let record = try? keyPair.data() else {
    ...
    private func createKeyPair(publicKey: [UInt8], privateKey: [UInt8]) -> KeyPair? {
        guard let pub = try? PublicKey(point: publicKey),
            let priv = try? PrivateKey(point: privateKey) else {
                return nil
        }
    ...
    func testRatchetingSessionAsAlice() {
        ...
        let aliceIdentityPrivate: [UInt8] = [
    ...
    private let aliceBasePublic: [UInt8] = [
    ...
    private let bobIdentityPublic: [UInt8] = [
    ...
    private let aliceEphemeralPublic: [UInt8] = [
    ...
    private let aliceIdentityPublic: [UInt8] = [
    ...
    private let bobSignedPublic: [UInt8] = [
